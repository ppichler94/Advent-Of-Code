...
class PartA(Day):
    def parse(self, text, data):
        ...
        data.data = nog.Array(map_lines_filled)
    ...
    def move(self, data, direction_index):
        ...
        next_character = data.data[next_position]
        if next_character == " ":
            back_direction = [-x for x in direction]
            while data.data[next_position + back_direction] != " ":
...
class PartB(PartA):
    def part_config(self, data):
        max_map_length = max(upper for lower, upper in data.data.limits()) - 2
        ...
        data.wrapping.update(  # add symmetric edge change, with reversed directions
            ...
            for (from_map_y, from_map_x, from_dir), (to_map_y, to_map_x, to_dir) in wrapping_items
        )
    ...
    def move(self, data, direction_index):
        ...
        if next_character == " ":
            ...
            for _ in range(
                next_direction_index - direction_index
                if direction_index <= next_direction_index
                else next_direction_index + 4 - direction_index
            ):
                y_on_face, x_on_face = x_on_face, y_on_face
                x_on_face = data.edge_length - 1 - x_on_face
            next_position = nog.Position.at(
                (face_y * data.edge_length + y_on_face + 1), (face_x * data.edge_length + x_on_face + 1)
            )
            next_character = data.data[next_position]
        if next_character == "#":
            return True
